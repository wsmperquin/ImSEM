 ...
 */
function fresno_theme_setup(){
    /*
     * Make child theme available for translation.
     * Translations can be filed in the /languages/ directory.
     */
    load_child_theme_textdomain( 'fresno', get_stylesheet_directory() . '/languages' );	
	require get_stylesheet_directory() . '/inc/customizer/fresno-customizer-options.php';	
}
add_action( 'after_setup_theme', 'fresno_theme_setup' );

/**
 * Load assets.
 */

function fresno_theme_css() {
	wp_enqueue_style( 'fresno-parent-style', get_template_directory_uri() . '/style.css' );
	wp_enqueue_style('fresno-child-style', get_stylesheet_directory_uri() . '/style.css');
	wp_enqueue_style('fresno-default-css', get_stylesheet_directory_uri() . "/assets/css/theme-default.css" );
    wp_enqueue_style('fresno-bootstrap-smartmenus-css', get_stylesheet_directory_uri() . "/assets/css/bootstrap-smartmenus.css" ); 	
}
add_action( 'wp_enqueue_scripts', 'fresno_theme_css', 99);

/**
 * Import Options From Parent Theme
 *
 */
function fresno_parent_theme_options() {
...
add_action( 'after_switch_theme', 'fresno_parent_theme_options' );

/**
 * Fresh site activate
 *
 */
$fresh_site_activate = get_option( 'fresh_fresno_site_activate' );
if ( (bool) $fresh_site_activate === false ) {
	...
	update_option( 'fresh_fresno_site_activate', true );
}

/**
 * Custom Theme Script
*/
function fresno_custom_theme_css() {
	$fresno_testomonial_background_image = get_theme_mod('arilewp_testomonial_background_image');
	?>
    <style type="text/css">
		<?php if($fresno_testomonial_background_image != null) : ?>
		.theme-testimonial { 
		        background-image: url(<?php echo esc_url( $fresno_testomonial_background_image ); ?>); 
...
<?php }
add_action('wp_footer','fresno_custom_theme_css');

/**
 * Page header
 *
 */
function fresno_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'fresno_custom_header_args', array(
		...
		'flex-height'        => true,
		'flex-width'         => true,
		'wp-head-callback'   => 'fresno_header_style',
	) ) );
}

add_action( 'after_setup_theme', 'fresno_custom_header_setup' );

/**
 * Custom background
 *
 */
function fresno_custom_background_setup() {
	add_theme_support( 'custom-background', apply_filters( 'fresno_custom_background_args', array(
		'default-color' => '202020',
		'default-image' => '',
	) ) );
}
add_action( 'after_setup_theme', 'fresno_custom_background_setup' );


if ( ! function_exists( 'fresno_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see fresno_custom_header_setup().
	 */
	function fresno_header_style() {
